...
export const exportAsPDF = async (elementId, filename = 'FixlyHandy_Report') => {
...
export const exportAsExcel = (data, filename = 'FixlyHandy_Report') => {
...
export const exportAsImage = async (elementId, filename = 'FixlyHandy_Report') => {
  const element = document.getElementById(elementId);
  if (!element) throw new Error('Export element not found');
...
export const printReport = async (elementId) => {
  ...
  printWindow.document.write('<html><head><title>FixlyHandy Report</title><style>body{margin:0;display:flex;justify-content:center;}img{max-width:100%;height:auto;}@media print{@page{margin:10mm;}}</style></head><body><img src="' + imgData + '"/><script>window.onload=()=>{window.print();window.onafterprint=()=>window.close();}<\/script></body></html>');
